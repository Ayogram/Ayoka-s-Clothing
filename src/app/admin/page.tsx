                ...
                {MOCK_PRODUCTS.map((product) => (
                  <div key={product.id} className="group relative flex flex-col space-y-4 border border-gray-50 dark:border-zinc-900 p-4 hover:border-gold-500/30 transition-all">
                    <div className="relative aspect-[3/4] overflow-hidden bg-gray-50 dark:bg-zinc-900">
                      <img src={product.mainImage} alt={product.name} className="object-cover w-full h-full grayscale group-hover:grayscale-0 transition-all duration-700" />
               ...
               <div className="p-8 border-b border-gray-50 dark:border-zinc-900">
                  <div className="flex flex-wrap gap-4">
                    {["All Orders", "Pending", "Processing", "Shipped", "Delivered"].map((status) => (
                      <button key={status} className="px-6 py-2 border border-gray-100 dark:border-zinc-800 text-[9px] uppercase tracking-widest font-bold hover:gold-text transition-all">
                        {status}
                      </button>
                    ))}
                  </div>
                 ...
                 <table className="w-full text-left">
                   <thead className="bg-gray-50/50 dark:bg-zinc-900/50 border-b border-gray-100 dark:border-zinc-900">
                     <tr>
                       ...
                       <th className="px-8 py-4 text-[10px] uppercase tracking-widest text-gray-400 font-bold">Date</th>
                   ...
                   <tbody className="divide-y divide-gray-50 dark:divide-zinc-900">
                      {[
                        { id: "AYK-8821", date: "24 Mar 2026", user: "Tolu L.", method: "Bank Transfer", status: "Pending" },
                        { id: "AYK-8822", date: "24 Mar 2026", user: "Becca S.", method: "Bank Transfer", status: "Processing" },
                        { id: "AYK-8823", date: "23 Mar 2026", user: "Damilola O.", method: "Bank Transfer", status: "Shipped" },
                      ].map((order) => (
                        <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-zinc-950 transition-colors">
                          <td className="px-8 py-6 text-xs font-bold">{order.id}</td>
                          <td className="px-8 py-6 text-xs text-gray-500">{order.date}</td>
                          <td className="px-8 py-6 text-xs">{order.user}</td>
                          <td className="px-8 py-6 text-[10px] uppercase font-bold tracking-tighter">{order.method}</td>
                          <td className="px-8 py-6">
                            <span className={`text-[9px] uppercase tracking-widest font-bold px-3 py-1 border ${
                              order.status === 'Pending' ? 'text-red-500 border-red-100' : 'text-gold-500 border-gold-100'
                            }`}>
                              {order.status}
                            </span>
                          </td>
                          <td className="px-8 py-6 text-right">
                             <button className="text-[10px] uppercase tracking-widest gold-text font-bold hover:underline">Update</button>
                          </td>
                        </tr>
                      ))}
                   </tbody>
            ...
            <div className="bg-white dark:bg-black border border-gray-100 dark:border-zinc-900 p-8 shadow-sm max-w-2xl">
              <div className="space-y-8">
                <div className="space-y-2">
                  ...
                  <input type="text" defaultValue="Ayoka Clothings" className="w-full bg-transparent border-b border-gray-100 dark:border-zinc-800 py-3 text-sm outline-none focus:border-gold-500" />
                </div>
                <div className="space-y-2">
